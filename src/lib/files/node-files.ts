...
import { readImportedFiles } from './imported-files';
...
export function nodeFiles(node: Node): FileSystemTree {
	// What an import said this node runs, until the mount that lays it down
	const imported = readImportedFiles(node.id);
	if (imported) return fileTree(imported);
