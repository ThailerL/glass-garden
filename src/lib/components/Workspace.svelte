...
<script lang="ts">
	...
	const {
		...
		// trigger floating over its content. Ignored on the desktop layout, which has neither
		topBar?: Snippet;
...
{#if isMobile.current}
	...
	<Sheet.Root bind:open={menuOpen}>
		<Sheet.Content side="left" class="w-72 gap-0 p-0 sm:max-w-72" showCloseButton={false}>
