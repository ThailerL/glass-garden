...
describe('challengeSchema', () => {
	...
	it('refuses a goal with no conditions and a type that is not a resource', () => {
		...
		expect(() =>
			challengeOf({
				node: { ref: { name: 'App' }, config: { name: { eq: ['Jobs'] as unknown as string } } }
			})
		).toThrow();
	});
	...
	it('refuses a comparison that says nothing, which would hold for a missing setting', () => {
		expect(() => challengeOf({ node: { ref: { name: 'App' }, config: { nope: {} } } })).toThrow(
			/needs eq, gte or lte/
		);
		expect(
			goalIds(
...
describe('judge', () => {
	it('checks a node and its settings against the canvas the run began with', () => {
		const enough = challengeOf({
			node: { ref: { name: 'App' }, config: { instanceCount: { gte: 2 } } }
		});
		const tooMany = challengeOf({
			node: { ref: { name: 'App' }, config: { instanceCount: { gte: 3 } } }
		});
		const named = challengeOf({
			node: { ref: { type: 'sqsQueue' }, config: { name: { eq: 'Jobs' } } }
		});
