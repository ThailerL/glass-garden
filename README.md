...
A `data` condition asks what a resource holds, through one of the reads that kind of resource offers. A Table (DynamoDB) offers `item`, which takes the key of one item as its `args` and answers that item's attributes. A Bucket (S3) offers `objects`, which takes a `prefix` as its `args`, or no `args` at all for the whole bucket, and answers the `count` of what is under it. `has` compares what the read found, field by field, each one reading `eq`, `gte`, `lte`, or a mix, and a condition comparing nothing at all asks only that there is something there. A bucket answers its count even where nothing is under the prefix, so compare that count rather than leaving `has` out. Only a field holding a string, a number, or a boolean can be compared. The read is taken once, `at` that second of the run, or at its end where the goal says nothing, and the goal reads as still being judged until it answers. Every matching node has to satisfy it, and a `{ "type": … }` the reader never added fails, since there is nothing there to read. A read a resource does not offer, or arguments it cannot use, is refused as the file is read, and `schema.json` lists the names, so an editor offers them as you type.
