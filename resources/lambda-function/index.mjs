...
// the local AWS region has to start another to keep up.
//
// Saving this file deploys it, as uploading a package does on Lambda: the environments
// already running finish what they have and stop, and the next invocation cold-starts on
// the new code.
//
...
//   event.Records. The edge is an event source mapping: the region polls the queue on the
//   function's behalf, exactly as the Lambda service does, and deletes the batch from the
//   queue only when the handler returns. Throw instead, and the whole batch comes back after
//   the queue's visibility timeout and is delivered again.
